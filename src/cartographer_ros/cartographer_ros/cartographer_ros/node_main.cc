...
/**
 * gflags是一套命令行参数解析工具,DEFINE_bool在gflags.h中定义
...
void Run() {
  // 使用tf2定义了ROS的坐标系统监听器,用于监听ROS系统中发布的坐标变换(tf)消息,并将之缓存在tf_buffer中
  // kTfBufferCacheTimeInSeconds定义了缓存的时间长度
  constexpr double kTfBufferCacheTimeInSeconds = 10.;
  tf2_ros::Buffer tf_buffer{::ros::Duration(kTfBufferCacheTimeInSeconds)};
  tf2_ros::TransformListener tf(tf_buffer);
  
  // 根据Lua配置文件中的内容,为node_options和trajectory_options赋值
  ...
  // c++11:std::move是将对象的状态或者所有权从一个对象转移到另一个对象,
